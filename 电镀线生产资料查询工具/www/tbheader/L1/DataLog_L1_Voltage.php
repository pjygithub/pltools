...
// 执行SQL语句
$sql = 'SELECT * FROM `tb_tbheader` WHERE `L1` = "1"  AND `type` IN ("0","2")';
$DataLog_L1_Voltage = array();
$results_nick_col = $db_handle->query($sql);
while ($row = $results_nick_col->fetchArray()) {
    $tbheader = $row["tbheader"];
    array_push($DataLog_L1_Voltage, $tbheader);
}
// var_dump($DataLog_L1_Voltage);
$db_handle->close();
$tbheader = $DataLog_L1_Voltage;
