...
// 执行SQL语句
$sql = 'SELECT * FROM `tb_tbheader` WHERE `L4` = "1" AND `type` IN ("0","10")';
$DataLog_L4_Pressure = array();
$results_nick_col = $db_handle->query($sql);
while ($row = $results_nick_col->fetchArray()) {
    $tbheader = $row["tbheader"];
    array_push($DataLog_L4_Pressure, $tbheader);
}
// var_dump($DataLog_L4_Pressure);
$db_handle->close();
$tbheader = $DataLog_L4_Pressure;
