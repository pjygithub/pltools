<?
$DataLog_mainPlatingTankCurr = array();
...
// 执行SQL语句
$sql = 'SELECT * FROM `tb_tbheader` WHERE `main` = "1"';
$DataLog_mainPlatingTankCurr = array();
$results_nick_col = $db_handle->query($sql);
while ($row = $results_nick_col->fetchArray()) {
    $tbheader = $row["tbheader"];
    array_push($DataLog_mainPlatingTankCurr, $tbheader);
}
// var_dump($DataLog_mainPlatingTankCurr);
$db_handle->close();
$tbheader = $DataLog_mainPlatingTankCurr;
