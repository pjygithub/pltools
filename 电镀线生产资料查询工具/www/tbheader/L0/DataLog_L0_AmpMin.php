...
// 执行SQL语句
$sql = 'SELECT * FROM `tb_tbheader` WHERE `L0` = "1" AND `type` IN ("8","0")';
$DataLog_L0_AmpMin = array();
$results_nick_col = $db_handle->query($sql);
while ($row = $results_nick_col->fetchArray()) {
    $tbheader = $row["tbheader"];
    array_push($DataLog_L0_AmpMin, $tbheader);
}
// var_dump($DataLog_L0_AmpMin);
$db_handle->close();
$tbheader = $DataLog_L0_AmpMin;
