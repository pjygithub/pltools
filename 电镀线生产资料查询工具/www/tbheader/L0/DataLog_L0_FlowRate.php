...
// 执行SQL语句
$sql = 'SELECT * FROM `tb_tbheader` WHERE `L0` = "1" AND `type` IN ("0","9")';
$DataLog_L0_FlowRate = array();
$results_nick_col = $db_handle->query($sql);
while ($row = $results_nick_col->fetchArray()) {
    $tbheader = $row["tbheader"];
    array_push($DataLog_L0_FlowRate, $tbheader);
}
// var_dump($DataLog_L0_FlowRate);
$db_handle->close();
$tbheader = $DataLog_L0_FlowRate;
