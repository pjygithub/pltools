...
<body>
    ...
    if ($machines_arr[$index][$dirname] != 1) {
        // echo '<p style="width:60%;margin:5% auto;font-size:1.5rem;">该机台似乎不存在' . $dirname . '这样的生产资料！</p>';
        // die;
    }
    try {
        if (url_exist("http://" . $machines_arr[$index]['host'] . "/")) {
            $url = "http://" . $machines_arr[$index]['host'] . "/" . $dirname . "Viewer/index.php";
        } else {
        ...
        try {
            if (url_exist("http://" . $machines_arr[$index]['ip'] . "/")) {
                $url = "http://" . $machines_arr[$index]['ip'] . "/" . $dirname . "Viewer/index.php";
            } else {
